...
function d3_rebind(target, source, method) {
  ...
  d3.brush.multibrush = function(options_in) {
    ...
    event.of = function(thiz, argumentz) {
      return function(e1) {
        try {
          ...
          e1.target = brush;
          d3.event = e1;
          event.apply(e1.type, thiz, argumentz);
        } finally {
          d3.event = e0;
        }
      };
    ...
    function brush(g) {
      g.each(function() {
        ...
        var gUpdate = g,//d3.transition(g),
            backgroundUpdate = g.selectAll(".background"),//d3.transition(g.selectAll("rect.background")),
            range;
    ...
    function brushstart() {
      var target = this,
          ...
          dragging = options_in && options_in.disableDragging ? false : eventTarget.classed("extent"),
      ...
      function keydown() {
        if (d3.event.keyCode == 32) {
          if (!dragging) {
            ...
            dragging = options_in && options_in.disableDragging ? false : 2; 
          }
          d3.event.preventDefault();
        }
      }
      ...
      function keyup() {
        if (d3.event.keyCode == 32 && dragging == 2) {
          origin[0] += xExtent[i][1];
          origin[1] += yExtent[i][1];
          dragging = options_in && options_in.disableDragging ? false : 0;
          d3.event.preventDefault();
        }
      }
